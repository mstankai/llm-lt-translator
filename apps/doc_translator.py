...
def system_prompt(language: str) -> str:
    return f"""
    You are a skilled {language} language translator. Your goal is to provide
    translations that are: Accurate: Convey the exact meaning of the original text,
    including idioms and cultural references. Fluent: Ensure the translation reads
    naturally and smoothly in the target language. Culturally Appropriate:
    Adapt the text for the target audience while respecting cultural differences.
    Consistent: Maintain tone, style, and terminology throughout the translation.
    Context-Aware: Understand and accurately translate technical or specialized
    terms relevant to the text's field. This is really important:
    make sure to keep the delimiter {DELIMITER} in the same
    places as in the original text. Make sure there is the same number of delimiters
    in the output as in the input. Please translate the following text from English
    to {language}, keeping the delimiter {DELIMITER} between the text blocks:
    """
...
def get_cost_of_tokens(
    n_tokens: int, model_name: str, price_list: dict, is_input: bool = True
) -> float:
...
def get_completion(client: OpenAI, messages: list, model_name: str) -> str:
    ...
    completion_input = dict(model=model_name, messages=messages)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Translate a PDF file to a selected language"
    )
    parser.add_argument("input")
    parser.add_argument("-o", "--output", default=None)
    parser.add_argument(
        "-m", "--model", default="gpt-4o-mini", choices=available_models
    )
    ...
    input_cost = get_cost_of_tokens(
        n_input_tokens, model_name, price_list, is_input=True
    )
    ...
    if n_input_tokens > MAX_TOKENS:
        ValueError(
            f"Input tokens ({n_input_tokens}) exceed the maximum of {MAX_TOKENS}."
        )
